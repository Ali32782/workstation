...
FILE_PATTERNS = {
    ...
    "hyrox_schedule":   "*yrox*schedules*.xlsx",          # SportsNow: Kursplan Export
...
def load_umsatzanalyse(path: Path) -> dict:
    ...
    if "Übersicht" in ua:
        ...
        for i in range(4, len(ov)):
            ...
            if not name or name in ["xxx","Summe","Check","NaN","nan",""]:
                # Auch Summen-Zeilen aus rechtem Teil lesen
                label = str(row[29]).strip() if pd.notna(row[29]) else ""
                if label == "FTE":
                    overview["fte_jan"] = row[30] if pd.notna(row[30]) else None
                    overview["fte_feb"] = row[31] if pd.notna(row[31]) else None
                elif label == "Ferien":
                    overview["ferien_jan"] = row[30] if pd.notna(row[30]) else None
                    overview["ferien_feb"] = row[31] if pd.notna(row[31]) else None
                elif label == "Umsatz":
                    overview["umsatz_jan"] = row[30] if pd.notna(row[30]) else None
                    overview["umsatz_feb"] = row[31] if pd.notna(row[31]) else None
                elif label == "Umsatz pro FTE":
                    overview["umsatz_fte_jan"] = row[30] if pd.notna(row[30]) else None
                    overview["umsatz_fte_feb"] = row[31] if pd.notna(row[31]) else None
                elif label == "Zielerreichung Ø":
                    overview["ziel_jan"] = row[30] if pd.notna(row[30]) else None
                    overview["ziel_feb"] = row[31] if pd.notna(row[31]) else None
                elif label == "Krank & andere":
                    overview["krank_jan"] = row[30] if pd.notna(row[30]) else None
                    overview["krank_feb"] = row[31] if pd.notna(row[31]) else None
                continue
...
def load_hyrox(schedule_path: Path, active_path: Path, past_path: Path) -> dict:
    """
    Liest Hyrox-Daten aus SportsNow-Exporten:
      - schedule_path: Kursplan-Export (Stundenplan ohne TN-Details)
      - active_path:   Aktive Abonnemente
      - past_path:     Abgelaufene/vergangene Abonnemente
    """
    result = {
    ...
    if schedule_path and schedule_path.exists():
        print(f"  Lese Hyrox Kursplan: {schedule_path.name}")
        sc = pd.read_excel(schedule_path, header=0)
        ...
        for ws_date, grp in sc.groupby("WeekStart"):
            ...
            result["weekly"].append({
                ...
                "neue_kunden": 0,  # nicht im Kursplan, kommt aus Passes
    ...
    if all_passes:
        ...
        # Wer hat Trial UND 10er/30er gekauft?
...
def write_uebersicht(wb, ua_data, rl_data, hy_data, inv_data, fit_data, today_str):
    ...
    a_rows = [
        ...
        ("Umsatz pro FTE (CHF)", ov.get("umsatz_fte_jan",0) or 0, ov.get("umsatz_fte_feb",0) or 0,
         ((ov.get("umsatz_fte_jan",0) or 0)+(ov.get("umsatz_fte_feb",0) or 0))/2, "#'##0.00"),
        ("Ø Zielerreichung (B)", jan_zB, feb_zB, (jan_zB+feb_zB)/2, "0.0%"),
        ("Krankheitstage", ov.get("krank_jan",0) or 0, ov.get("krank_feb",0) or 0,
         (ov.get("krank_jan",0) or 0)+(ov.get("krank_feb",0) or 0), "0"),
        ("Monate mit Daten", len([m for m in months_avail if "26" in m]), None, None, "0"),
    ]
    for i, (label, jv, fv, tot, fmt) in enumerate(a_rows, 10):
        bg = ALT if i%2==0 else WHITE
        dc(ws,i,1,label,bg,bold=True,h="left")
        dc(ws,i,2,jv,bg,fmt=fmt) if jv is not None else dc(ws,i,2,"–",bg,h="center")
        dc(ws,i,3,fv,bg,fmt=fmt) if fv is not None else dc(ws,i,3,"–",bg,h="center")
        dc(ws,i,4,tot,bg,bold=True,fmt=fmt) if tot is not None else dc(ws,i,4,"–",bg,h="center")
    gap(ws, 16, 12)

    # B – Runnerslab Jahresvergleich gleicher Zeitraum
    sec(ws, 17, "B │ RUNNERSLAB – Jan+Feb Jahresvergleich", cols=12)
    hdr(ws, 18, ["Jahr","Jan Total","Feb Total","Jan+Feb","Δ% vs. Vorjahr"])
    years_to_show = sorted([y for y in rl_data.keys() if y >= 2021])
    prev_jf = None
    for i, yr in enumerate(years_to_show, 19):
        bg = ALT if i%2==0 else WHITE
        jan_t = rl_data[yr].get("Jan",{}).get("total",0)
        feb_t = rl_data[yr].get("Feb",{}).get("total",0)
        jf = jan_t + feb_t
        dc(ws,i,1,str(yr),bg,bold=True,h="center")
        dc(ws,i,2,jan_t,bg,fmt="#'##0"); dc(ws,i,3,feb_t,bg,fmt="#'##0")
        dc(ws,i,4,jf,bg,bold=True,fmt="#'##0")
        if prev_jf and prev_jf > 0:
            delta = (jf - prev_jf) / prev_jf
            c = ws.cell(i,5,delta); c.number_format="0.0%"; c.fill=fill(bg)
            c.font=Font(name="Arial",bold=True,size=10,color=GREEN if delta>0 else RED)
            c.border=bb(); c.alignment=aln("right")
        else:
            dc(ws,i,5,"–",bg,h="center")
        prev_jf = jf
    gap(ws, 19+len(years_to_show), 12)

    # C – Runnerslab Monat für Monat
    r_c = 20+len(years_to_show)
    ...
    for i, m in enumerate(months_s):
        r = r_c+2+i; bg = ALT if r%2==0 else WHITE
        dc(ws,r,1,m,bg,bold=True,h="left")
        vals = []
        for j, yr in enumerate(show_years):
            v = rl_data[yr].get(m,{}).get("total",0) or 0
            dc(ws,r,j+2,v if v else None,bg,fmt="#'##0",bold=(j==len(show_years)-1))
            vals.append(v)
        if len(vals) >= 2 and vals[-2] and vals[-1]:
            d = vals[-1]-vals[-2]; dp = d/vals[-2]
            c1=ws.cell(r,len(show_years)+2,d); c1.number_format="#'##0"; c1.fill=fill(bg)
            c1.font=Font(name="Arial",bold=True,size=10,color=GREEN if d>0 else RED)
            c1.border=bb(); c1.alignment=aln("right")
            c2=ws.cell(r,len(show_years)+3,dp); c2.number_format="0.0%"; c2.fill=fill(bg)
            c2.border=bb(); c2.alignment=aln("right")
        else:
            for c_ in [len(show_years)+2, len(show_years)+3]:
                dc(ws,r,c_,"–",bg,h="center")

    for c,w in [(1,22),(2,14),(3,14),(4,14),(5,14),(6,12),(7,10),(8,10),(9,10),(10,8),(11,8),(12,8)]:
...
def main():
    ...
    if hy_sched or hy_active or hy_past:
        hy_data = load_hyrox(hy_sched, hy_active, hy_past)
    ...
    desired = ["📊 Übersicht","👥 Therapeuten","👟 Runnerslab","🏃 Hyrox",
               "🏋 Fitness-Abos","💳 Ausstehende Zhlg.","⚖️ Inkasso (manuell)","📋 Berichte Ärzte"]
