...
def load_hyrox(schedule_path: Path, active_path: Path, past_path: Path) -> dict:
    ...
    if schedule_path and schedule_path.exists():
        ...
        # Prüfen ob es der richtige Export ist (Kursplan mit Auslastung, nicht Teilnehmerliste)
        if "Anmeldungen" not in sc.columns:
            # Das ist die Teilnehmerliste — wir brauchen den anderen Export
            # Versuche alle schedule-Dateien und nimm die mit "Anmeldungen"
            all_schedules = find_all_files("hyrox_schedule")
            sc = None
            for alt_path in all_schedules:
                alt = pd.read_excel(alt_path, header=0)
                alt.columns = alt.columns.str.strip()
                if "Anmeldungen" in alt.columns:
                    print(f"  → Verwende Kursplan: {alt_path.name}")
                    sc = alt
                    break
            if sc is None:
                print("  ⚠️  Kein Kursplan-Export gefunden (brauche Export mit Spalte 'Anmeldungen')")
                print("     Bitte in SportsNow: Stundenplan → Exportieren (ohne Teilnehmerliste)")
                sc = pd.DataFrame()
