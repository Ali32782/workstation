...
def practice_to_company_input(practice: dict, tenant: str) -> dict[str, Any]:
    ...
    Required custom fields on Company (create them once in Twenty Settings →
    Data Model → Companies → '+ Field'):
        tenant (Text), leadSource (Text), bookingSystem (Text),
        googleRating (Number), googleReviewCount (Number),
        employeeCountPhysio (Number), specializations (Text),
        languages (Text), generalEmail (Text), phone (Text),
        ownerName (Text), ownerEmail (Text), ownerSource (Text),
        leadTherapistName (Text), leadTherapistEmail (Text),
        teamMembersJson (Text — full team roster as JSON string)
    ...
    return {
        ...
        "bookingSystem": practice.get("booking_system") or "none",
...
def _person_input(name: str | None, email: str | None, phone: str | None,
                  ...
                  practice_role_label: str | None = None) -> dict[str, Any] | None:
    """
    ...
    Required Person custom fields in Twenty:
        ...
        practiceRole (Text — original role string from website like
                              'Physiotherapeutin SRK'),
        guessedEmail (Text — comma-separated guesses, prefixed 'guess:')
